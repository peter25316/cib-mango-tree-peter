...
class UnifiedAnalysisPipeline:
    ...
    def __init__(self, data_path: str, cache_dir: str = 'cache', plots_dir: str = 'plots'):
    ...
    def load_cached_results(self) -> bool:
        """Try to load cached results to skip expensive operations"""
        try:
            cache_file = os.path.join(self.cache_dir, 'unified_pipeline_cache.pkl')
            if os.path.exists(cache_file):
                with open(cache_file, 'rb') as f:
                    cached = pickle.load(f)

                # Check if cache is for same data file
                if cached.get('data_path') == self.data_path:
                    self.analyzer = cached.get('analyzer')
                    self.results = cached.get('results', {})
                    self.timing = cached.get('timing', {})
    ...
    def run_data_analysis(self, force_rerun: bool = False) -> bool:
        """Run data analysis step"""
        if not force_rerun and 'data_analysis' in self.results:
            print("📊 Using cached data analysis results")
            return True
    ...
    def generate_visualizations(self, force_rerun: bool = False) -> bool:
        """Generate all visualizations"""

        if not force_rerun and 'visualizations' in self.results:
            print("📊 Using cached visualization results")
            return True
        ...
        if 'coordination_analysis' in self.results:
            try:
                ...
                # Create coordination networks visualization if networks exist
                if coordination_results.get('coordination_networks'):
                    viz.plot_coordination_networks(
                        coordination_results,
                        save_path=os.path.join(self.plots_dir, 'coordination_networks.png')
                    )
                    print("   ✅ Coordination networks visualization")
                    vis_results['coordination_networks'] = True
                else:
                    print("   ⚠️ No coordination networks detected to visualize")
    ...
    def run_temporal_clustering(self, force_rerun: bool = False) -> bool:
        """Run temporal clustering analysis"""

        if not force_rerun and 'temporal_clustering' in self.results:
    ...
    def run_complete_pipeline(self, burst_params: Dict = None, force_rerun: bool = False) -> bool:
        ...
        if not force_rerun and self.load_cached_results():
            print("🎯 Pipeline completed using cached results")
            self.print_summary()
            return True

        steps = [
            ('Data Analysis', lambda: self.run_data_analysis(force_rerun)),
            ...
            ('Coordination Analysis', lambda: self.run_coordination_analysis(force_rerun)),
            ('Temporal Clustering', lambda: self.run_temporal_clustering(force_rerun)),
            ('Visualization', lambda: self.generate_visualizations(force_rerun))
        ]
...
if __name__ == "__main__":
    try:
        ...
        if success:
            ...
            print("="*60)
            print("\n💡 Results have been cached and visualizations generated.")
            print("📁 Check the 'plots' directory for all visualizations.")
            print("📊 Check the 'cache' directory for cached results.")
            print("\n🔍 Use the launcher.py to access detailed reports and analysis.")
